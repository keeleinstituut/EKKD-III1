...
Usage:
    uv run python 04-ghost_screen.py [--rare-threshold 500]
"""
...
# Run from the working-repo root. 2026-08-10: repointed from the old draft-folder
# copies to the canonical master.
MASTER = Path("Katus-ALUSANDMED/json-all/AMT-Master_annotated.json")
# Third-party modern-Estonian lemma frequency list (lemma<TAB>frequency, one
# "lemma-POS" per line); archived in the working repo, not distributed here.
FREQ_FILE = Path("lemmafreq-jan24.txt")
OUT_CSV = Path("lookup-needed.csv")  # worklist for the Sõnaveeb/ÜS lookup step
