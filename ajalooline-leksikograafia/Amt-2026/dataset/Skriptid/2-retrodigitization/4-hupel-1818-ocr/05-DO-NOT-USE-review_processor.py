#!/usr/bin/env python3
# Created: 2026-03-18 09-53-39
# Author: Madis Jürviste
"""
Batch Review Processor for Hupel's Estonian-German Dictionary (1818 Edition)

DO NOT USE: this later (2026-03-18) variant ran the review pass on Claude
Sonnet 4.6 with extended thinking disabled, as a cost experiment; its output
did not meet the quality bar of the review_processor_co46te.py run (Opus 4.6
with thinking) and was rejected. Kept to document the model comparison.

This script reviews and corrects the initial OCR output by comparing it
against the original scanned images using Claude Sonnet 4.6 via the Batch API.
Each page's raw OCR text is paired with its scanned JPG image, the review prompt,
the main OCR prompt (for reference), and a CAT example file as ground truth.

Usage:
    python review_processor.py --pages 15-20,25,30-35
    python review_processor.py --pages 55 --dry-run
    python review_processor.py --batch-ids msgbatch_XXX,msgbatch_YYY

Requirements:
    - ANTHROPIC_API_KEY environment variable must be set
    - Raw OCR output in output-raw/ folder
    - Input JPGs in input-ADA/input-JPG/ folder
    - Corrected output written to output-review/ folder
"""
...
INPUT_DIR = BASE_DIR / "input-ADA" / "input-JPG"
RAW_OUTPUT_DIR = BASE_DIR / "output-raw"
REVIEW_OUTPUT_DIR = BASE_DIR / "output-review"
...
# Pricing: Claude Sonnet 4.6 Batch API (50% discount on standard rates)
# Standard: $3.00 input / $15.00 output per MTok
# Batch:    $1.50 input /  $7.50 output per MTok
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(BASE_DIR / "review_processor.log"),
    ],
)
...
def get_output_filename(raw_filename: str) -> str:
    ...
    Example: 015_Hu-1818_20260221_c-o-4-6_raw.txt → 015_Hu-1818_20260221_c-o-4-6_reviewed.txt
    """
...
def renumber_entry_ids(text: str, page_number: int) -> str:
    ...
    - 6818 = source prefix (6) + edition (818)
    - PPP = page number (zero-padded to 3 digits)
    - NNN = entry sequence on page (zero-padded to 3 digits)
...
def estimate_message_text_len(review_prompt: str, main_prompt: str, cat_example: str) -> int:
    ...
    # The message text contains: review prompt + main prompt + CAT example + raw OCR (varies)
    # Use a generous estimate for the raw OCR portion (~50 KB per page)
    sample = (
    ...
    for bid in batch_ids:
        ...
        for result in client.messages.batches.results(bid):
            ...
            if result.result.type == "succeeded":
                ...
                # Extract text content (skip thinking blocks if any)
                response_text = ""
                ...
                if page_num in raw_files:
                    ...
                    # Fallback naming if raw file not found
...
def log_summary(batch_ids: list[str], page_stats: dict, failed_pages: list[int]):
    ...
    logger.info(f"  Thinking : disabled")
    logger.info(f"  Pages    : {len(page_stats)} succeeded, {len(failed_pages)} failed")
    logger.info("")
    logger.info("-" * 80)
    logger.info("PER-PAGE PRICING BREAKDOWN (Claude Sonnet 4.6 Batch API)")
    logger.info(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Batch Review Processor for Hupel's Estonian-German Dictionary (1818 Edition) — Sonnet 4.6"
    )
    parser.add_argument(
        "--pages",
        type=str,
    ...
    parser.add_argument(
        "--input-dir",
        type=str,
        help="Custom input image directory (default: input-ADA/input-JPG)",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    # Resolve custom input directory
    custom_input_dir = Path(args.input_dir) if args.input_dir else None
    if custom_input_dir and not custom_input_dir.is_dir():
        parser.error(f"Input directory not found: {custom_input_dir}")

    # Resolve input files if --pages provided
    input_files = []
    if args.pages:
        requested_pages = parse_pages_arg(args.pages)
        input_files = resolve_input_files(requested_pages, custom_input_dir)
