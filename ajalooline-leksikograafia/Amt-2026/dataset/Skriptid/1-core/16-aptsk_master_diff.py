...
# Run from the original working-repo root. APT_DIR points at the json/ output
# of the aptsk-convert pipeline (see retrodigitization/aptsk-convert/), which
# lives in a separate working repo; override via the first CLI argument.
import sys as _sys
MASTER = Path("Katus-ALUSANDMED/json-all/AMT-Master_annotated.json")
APT_DIR = Path(_sys.argv[1]) if len(_sys.argv) > 1 else Path("../APTSK-analysis/json")
OUT_DIR = Path("Katus-ALUSANDMED")
...
lines.append(f"Generated by `16-aptsk_master_diff.py`.\n")
