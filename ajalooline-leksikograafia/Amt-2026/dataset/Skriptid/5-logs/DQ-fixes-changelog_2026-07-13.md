...
> **Legend (added 2026-08-28 for the public release; the log itself is unedited below this line).**
> **T1–T20** = tasks of the 2026-07-12 data-quality tasklist (a working document not included in this release). **D1–D12** = author decisions accompanying that tasklist, enumerated in *Run scope* above. **B1–B9 / C1–C5 / E… / F… / J… / K…** = item codes of the 2026-07-12 review documents (working documents, not included); e.g. J1 = extraction of all non-NULL comments into a separate archive file. **"kappa" / katuspeatükk** = the dissertation's framing chapter. **MJ** = Madis Jürviste. **CSC** = `Cross-source count`. **ÜS** = EKI ühendsõnastik (Sõnaveeb). Referenced working files (`DQ-tasklist…`, `WIP-numbers…`, `Comments-in-JSON…`, `Katus-DRAFTS/`, `Katus-tervik-WIP/`) belong to the private working repository.
