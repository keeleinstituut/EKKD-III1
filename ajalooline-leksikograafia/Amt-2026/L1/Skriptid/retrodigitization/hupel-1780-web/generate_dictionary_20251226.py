#!/usr/bin/env python3
# Created: 2025-12-26 13-55-03
# Author: Madis Jürviste
"""Dictionary generator with improved formatting - Version 2025-12-26"""
import re
...
def process_xr_content(content, entry_id_map):
    ...
    with exclusions for 's.', 's. auch', 'f.', and 'f. auch'"""

    # Check for pattern with brackets like "s. auch woöraspid. [woöraspiddo]" or "s. woöraspid. [woöraspiddo]"
    bracket_match = re.match(r'([sf]\.(?:\s+auch)?)\s+(.+?)\s*\[([^\]]+)\]', content)
    if bracket_match:
    ...
    # Pattern without brackets like "s. auch word" or "s. word"
    match = re.match(r'([sf]\.(?:\s+auch)?)\s+(.+)', content)
    if match:
